...
export default async function handler(req, res) {
    ...
    if (req.method === "POST") {
        ...
        form.parse(req, async (err, fields, files) => {
            ...
            if (files.file.size / 1000 > imageSizeLimit) {
                return res.status(400).send(new Return422([
                    {
                        "msg": "Upload web small image image under 200kb",
                        "param": "file",
                        "location": "body"
                    }
                ]))
            ...
            try {
                ...
                if (dimension.width > imageWidth) {
                    ...
                    return res.status(400).send(new Return422([
                        {
                            "msg": `"Web small image width should be ${imageWidth}px`,
                            "param": "file",
                            "location": "body"
                        }
                    ]))
                ...
                if (dimension.height > imageHeight) {
                    ...
                    return res.status(400).send(new Return422([
                        {
                            "msg": `"Web small image height should be ${imageHeight}px`,
                            "param": "file",
                            "location": "body"
                        }
                    ]))
                }
                await galleryModel.findByIdAndUpdate(id, {smallWebImage: "https://18-candleriggs.fra1.digitaloceanspaces.com/" + t}, {new: true})
                await galleryModel.findByIdAndUpdate(id, {smallWebImageName: t}, {new: true})
        ...
        if(!findGallery.smallWebImageName){
            return res.status(404).send(new Return404("No image found to delete"))
        }
        if(Object.values(findGallery.smallWebImageName).length<1){
            return res.status(404).send(new Return404("No image found to delete"))
        }
        let imageToDelete = findGallery.smallWebImageName
        s3Client.deleteObject({Bucket: '18-candleriggs', Key: imageToDelete}, (err, data) => {
            console.error(err);
            console.log(data);
        })
        await galleryModel.findByIdAndUpdate(id, {smallWebImage: ""}, {new: true})
        await galleryModel.findByIdAndUpdate(id, {smallWebImageName: ""}, {new: true})
