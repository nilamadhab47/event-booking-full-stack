...
export default async function handler(req, res) {
    ...
    if (req.method === "POST") {
        ...
        form.parse(req, async (err, fields, files) => {
            ...
            if (files.file.size > imageSizeLimit) {
                return res.status(400).send(new Return422([
                    {
                        "msg": "Upload Mobile image small under 200kb",
                        "param": "file",
                        "location": "body"
                    }
                ]))
            ...
            try {
                ...
                await galleryModel.findByIdAndUpdate(id, {smallMobileImage: "https://18-candleriggs.fra1.digitaloceanspaces.com/" + t}, {new: true})
                await galleryModel.findByIdAndUpdate(id, {smallMobileImageName: t}, {new: true})
        ...
        if(!findGallery.smallMobileImageName){
            return res.status(404).send(new Return404("No image found to delete"))
        }
        if(Object.values(findGallery.smallMobileImageName).length<1){
            return res.status(404).send(new Return404("No image found to delete"))
        }
        let imageToDelete = findGallery.smallMobileImageName
        s3Client.deleteObject({Bucket: '18-candleriggs', Key: imageToDelete}, (err, data) => {
            console.error(err);
            console.log(data);
        })
        await galleryModel.findByIdAndUpdate(id, {smallMobileImage: ""}, {new: true})
        await galleryModel.findByIdAndUpdate(id, {smallMobileImageName: ""}, {new: true})
