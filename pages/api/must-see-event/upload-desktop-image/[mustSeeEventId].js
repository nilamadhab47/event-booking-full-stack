...
export default async function handler(req, res) {
    ...
    if(req.method==="POST"){
        ...
        form.parse(req, async (err, fields, files) => {
            ...
            if (files.file.size > imageSizeLimit) {
                return res.status(400).send(new Return422([
                    {
                        "msg": "Upload must see event desktop image under 500kb",
                        "param": "file",
                        "location": "body"
                    }
                ]))
            ...
            try {
                ...
                if (dimension.width !== imageWidth) {
                    ...
                    return res.status(400).send(new Return422([
                        {
                            "msg": `Desktop must see event image width should be ${imageWidth}px`,
                            "param": "file",
                            "location": "body"
                        }
                    ]))
                ...
                if (dimension.height !== imageHeight) {
                    ...
                    return res.status(400).send(new Return422([
                        {
                            "msg": `Desktop must see event image height should be ${imageHeight}px`,
                            "param": "file",
                            "location": "body"
                        }
                    ]))
                }
                await mustSee.findByIdAndUpdate(id, {desktopImageName: t}, {new: true})
                await mustSee.findByIdAndUpdate(id, {desktopImage: "https://18-candleriggs.fra1.digitaloceanspaces.com/" + t}, {new: true})
        ...
        if(Object.values(findEvent.desktopImageName).length<2){
            return res.status(200).send({code: 200, msg: "OK"})
        }
        ...
        await mustSee.findByIdAndUpdate(id, {desktopImage: ""}, {new: true})
        await mustSee.findByIdAndUpdate(id, {desktopImageName: ""}, {new: true})
