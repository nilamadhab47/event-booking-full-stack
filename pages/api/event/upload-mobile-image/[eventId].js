        ...
        if (req.method === "POST") {
            ...
            form.parse(req, async (err, fields, files) => {
                ...
                if (files.file.size > imageSizeLimit) {
                    return res.status(422).send(new Return422([
                        {
                            "msg": "Upload mobile event image under 200kb",
                            "param": "file",
                            "location": "body"
                        }
                    ]))
                ...
                try {
                    ...
                    await eventModel.findByIdAndUpdate(id, {mobileImage: "https://18-candleriggs.fra1.digitaloceanspaces.com/" + t}, {new: true})
                    await eventModel.findByIdAndUpdate(id, {mobileImageName: t}, {new: true})
            ...
            if (Object.values(findEvent.mobileImageName).length < 1) {
                return res.status(200).send({code: 200, msg: "OK"})
            }
            ...
            await eventModel.findByIdAndUpdate(id, {mobileImage: ""}, {new: true})
            await eventModel.findByIdAndUpdate(id, {mobileImageName: ""}, {new: true})
